...
def carregar_usuarios():
    if os.path.exists(ARQ_USUARIOS):
        with open(ARQ_USUARIOS, "r", encoding="utf-8") as f:
            dados = json.load(f)
            # Garante que todo usuário carregado tenha a lista de amigos iniciada
            for u in dados:
                if "amigos" not in u:
                    u["amigos"] = []
            return dados
...
class RedeSocial(ctk.CTk):
    ...
    def tela_login(self):
        ...
        titulo = ctk.CTkLabel(frame_central, text="Bem-vindo de volta", font=("Segoe UI", 26, "bold"))
        titulo.pack(pady=(50, 5))

        subtitulo = ctk.CTkLabel(frame_central, text="Conecte-se à sua conta", font=("Segoe UI", 14), text_color="gray")
        subtitulo.pack(pady=(0, 30))

        self.login_usuario = ctk.CTkEntry(frame_central, width=300, height=45, placeholder_text="Usuário", corner_radius=10)
        self.login_usuario.pack(pady=10)

        self.login_senha = ctk.CTkEntry(frame_central, width=300, height=45, placeholder_text="Senha", show="*", corner_radius=10)
        self.login_senha.pack(pady=10)
        
        self.lbl_erro = ctk.CTkLabel(frame_central, text="", font=("Segoe UI", 12, "bold"), text_color=("#FF3333", "#FF4D4D"))
        self.lbl_erro.pack(pady=(5, 5))

        btn_entrar = ctk.CTkButton(frame_central, text="Entrar", width=300, height=45, font=("Segoe UI", 14, "bold"), corner_radius=10, command=self.fazer_login)
        btn_entrar.pack(pady=(25, 10))

        btn_cadastrar = ctk.CTkButton(frame_central, text="Criar nova conta", width=300, height=45, fg_color="transparent", hover_color=("#EAEAEA", "#2B2B2B"), text_color=("#1F6AA5", "#4AA3DF"), font=("Segoe UI", 14, "bold"), corner_radius=10, command=self.tela_cadastro)
        btn_cadastrar.pack()
    ...
    def tela_cadastro(self):
        ...
        ctk.CTkLabel(frame_central, text="Criar Conta", font=("Segoe UI", 26, "bold")).pack(pady=(50, 5))
        ctk.CTkLabel(frame_central, text="Junte-se à nossa comunidade hoje", font=("Segoe UI", 14), text_color="gray").pack(pady=(0, 30))

        self.cad_usuario = ctk.CTkEntry(frame_central, width=300, height=45, placeholder_text="Adicione o nome de Usuário", corner_radius=10)
        self.cad_usuario.pack(pady=10)

        self.cad_senha = ctk.CTkEntry(frame_central, width=300, height=45, placeholder_text="Registre sua Senha", show="*", corner_radius=10)
        self.cad_senha.pack(pady=10)

        btn_salvar = ctk.CTkButton(frame_central, text="Cadastrar", width=300, height=45, font=("Segoe UI", 14, "bold"), corner_radius=10, command=self.salvar_usuario)
        btn_salvar.pack(pady=(25, 10))

        btn_voltar = ctk.CTkButton(frame_central, text="Voltar para o Login", width=300, height=40, fg_color="transparent", text_color="gray", hover_color=("#EAEAEA", "#2B2B2B"), command=self.tela_login)
        btn_voltar.pack()
    ...
    def salvar_usuario(self):
        ...
        if any(u["usuario"] == usuario for u in usuarios) or usuario == "Suporte_Hub":
            return
    ...
    # GERENCIAMENTO DE AMIZADES REAL
    ...
    def deletar_post(self, post_alvo, de_perfil): 
        ...
        if camino_foto and os.path.exists(caminho_foto):
            try:
    ...
    def deletar_usuario(self):
        ...
        if usuario_alvo:
            # Remove este usuário das listas de amigos alheias
            for u in usuarios:
    ...
    def escolher_foto_perfil(self, label_status):
        arquivo_origem = filedialog.askopenfilename(title="Escolher foto de perfil", filetypes=[("Imagens", "*.png *.jpg *.jpeg *.gif")])
        if arquivo_origem:
            try: 
    ...
    def construir_layout_base(self, aba_ativa): 
        ...
        ctk.CTkLabel(topo, text="FP FacePit", font=("Segoe UI", 24, "bold"), text_color=("#1F6AA5", "#4AA3DF")).pack(side="left", padx=30)
        ctk.CTkEntry(topo, width=350, height=38, placeholder_text="Pesquisar na rede...", corner_radius=20, border_width=1).pack(side="left", padx=50)

        btn_sair = ctk.CTkButton(topo, text="Sair", width=70, height=35, fg_color=("#FF4D4D", "#CC3333"), hover_color=("#FF3333", "#992222"), font=("Segoe UI", 12, "bold"), corner_radius=10, command=self.tela_login)
        ...
        btn_toggle = ctk.CTkButton(menu, text=texto_hamburguer, height=35, width=40, font=("Segoe UI", 14, "bold"), fg_color="transparent", text_color="gray", hover_color=("#F0F2F5", "#2A2A2A"), command=self.alternar_menu)
        ...
        for texto_cheio, icone, comando, chave in botoes_menu:
            ...
            btn = ctk.CTkButton(
                menu, text=texto_exibir, anchor=alinhamento, height=40,
                ...
                hover_color=("#D0E6F7", "#28445C"), corner_radius=10, command=comando
            )
    ...
    def tela_feed(self):
        ...
        ctk.CTkLabel(caixa_post, text="No que você está pensando?", font=("Segoe UI", 14, "bold")).pack(anchor="w", padx=20, pady=(15, 5))
        self.post_texto = ctk.CTkTextbox(caixa_post, height=80, corner_radius=10, border_width=1, border_color=("#D0D0D0", "#333333"), font=("Segoe UI", 13))
        ...
        self.btn_foto = ctk.CTkButton(sub_barra, text="Adicionar Foto", width=140, height=35, fg_color=("#F0F2F5", "#2A2A2A"), text_color=("#000000", "#FFFFFF"), hover_color=("#E4E6EB", "#3A3A3A"), font=("Segoe UI", 12, "bold"), corner_radius=8, command=self.escolher_foto)
        self.btn_foto.pack(side="left")

        btn_publicar = ctk.CTkButton(sub_barra, text="Publicar", width=100, height=35, font=("Segoe UI", 12, "bold"), corner_radius=8, command=self.publicar_post)
    ...
    # TELA DE MENSAGENS (FILTRADA POR AMIGOS)
    def disparar_mensagem(self, destinatario, campo_texto):
        ...
        if destinatario == "Suporte_Hub":
            resposta_automatica = "Olá! Caso você deseje um tipo de suporte específico, entre em contato diretamente pelo número: (86)981835751"
            historico_mensagens.append({
    ...
    def tela_mensagens(self, contato_selecionado=None):
        ...
        # Carrega seus amigos reais do banco de dados para a barra de mensagens
        meu_perfil = next((u for u in usuarios if u["usuario"] == usuario_logado), {"amigos": []})
        meus_amigos = meu_perfil.get("amigos", [])

        # Lista de conversas: Canal fixo de suporte + somente amigos adicionados
        contatos = ["Suporte_Hub"] + meus_amigos
        ...
        for contato in contatos:
            ...
            prefixo = "🛠️" if contato == "Suporte_Hub" else "💬"
            btn_contato = ctk.CTkButton(
                coluna_esquerda, text=f"{prefixo}  {contato}", anchor="w", height=45,
        ...
        if not mensagens_filtradas:
            ...
            for msg in mensagens_filtradas:
                ...
                balao = ctk.CTkFrame(frame_balao, fg_color=("#1F6AA5", "#4AA3DF") if eh_meu else ("#F0F2F5", "#2A2A2A"), corner_radius=10)
                balao.pack(side="right" if eh_meu else "left", padx=5)

                ctk.CTkLabel(balao, text=msg["texto"], text_color="white" if eh_meu else ("#000000", "#FFFFFF"), font=("Segoe UI", 13), wraplength=350, justify="left").pack(padx=12, pady=8)
        ...
        btn_enviar = ctk.CTkButton(barra_envio, text="Enviar 🚀", width=90, font=("Segoe UI", 13, "bold"), corner_radius=10, command=lambda: self.disparar_mensagem(contato_selecionado, campo_msg))
        btn_enviar.pack(side="right", fill="y")

    # TELA DE AMIGOS ATUALIZADA
    def tela_amigos(self): 
        ...
        # SEÇÃO 1: SEUS AMIGOS
        # ---------------------------------------------------------
        ctk.CTkLabel(scroll_amigos, text="Seus Amigos", font=("Segoe UI", 18, "bold")).pack(anchor="w", padx=10, pady=(10, 15))

        if not meus_amigos:
            ctk.CTkLabel(scroll_amigos, text="Você ainda não adicionou nenhum amigo. 👥", font=("Segoe UI", 14, "italic"), text_color="gray").pack(anchor="w", padx=20, pady=10)
        else:
            for amigo in meus_amigos:
                ...
                btn_perfil_amigo = ctk.CTkButton( 
                    card_amigo, text=f"👤 {amigo}", font=("Segoe UI", 14, "bold"), text_color=("#000000", "#FFFFFF"),
                    fg_color="transparent", hover_color=("#F0F2F5", "#2A2A2A"), height=35,
                    command=lambda nome_amigo=amigo: self.tela_perfil(usuario_alvo=nome_amigo)
                )
                btn_perfil_amigo.pack(side="left", padx=15)

                ctk.CTkButton( 
                    card_amigo, text="Remover", width=90, height=30, fg_color="transparent", text_color=("#FF4D4D", "#FF4D4D"),
                    hover_color=("#FEE2E2", "#2D1F1F"), corner_radius=8,
                    command=lambda a=amigo: self.remover_amigo(a)
                ).pack(side="right", padx=20)

        # Separador de seções
        ctk.CTkFrame(scroll_amigos, height=2, fg_color=("#E0E0E0", "#2D2D2D")).pack(fill="x", padx=10, pady=25)

        # SEÇÃO 2: LISTA DE USUÁRIOS (SÓ QUEM NÃO É SEU AMIGO)
        ctk.CTkLabel(scroll_amigos, text="Lista de Usuários", font=("Segoe UI", 16, "bold"), text_color="gray").pack(anchor="w", padx=10, pady=(0, 15))

        # Filtra do banco quem não é você E quem ainda não é seu amigo
        usuarios_restantes = [u["usuario"] for u in usuarios if u["usuario"] != usuario_logado and u["usuario"] not in meus_amigos]

        if not usuarios_restantes:
            ctk.CTkLabel(scroll_amigos, text="Nenhum outro usuário disponível para adicionar.", font=("Segoe UI", 13, "italic"), text_color="gray").pack(anchor="w", padx=20)
        else:
            for sug in usuarios_restantes:
                ...
                btn_perfil_user = ctk.CTkButton(
                    card_sug, text=f"✨ {sug}", font=("Segoe UI", 14), text_color=("#000000", "#FFFFFF"),
                    fg_color="transparent", hover_color=("#F0F2F5", "#2A2A2A"), height=35,
                    command=lambda nome_u=sug: self.tela_perfil(usuario_alvo=nome_u)
                )
                btn_perfil_user.pack(side="left", padx=15)

                ctk.CTkButton(
                    card_sug, text="Adicionar", width=90, height=30, corner_radius=8,
                    command=lambda s=sug: self.adicionar_amigo(s)
                ).pack(side="right", padx=20)
    ...
    def tela_perfil(self, usuario_alvo=None): 
        ...
        if not eh_meu_perfil:
            btn_voltar_feed = ctk.CTkButton(frame_foto_nome, text="Voltar ao Feed", width=120, height=30, fg_color=("#E4E6EB", "#2A2A2A"), text_color=("#000000", "#FFFFFF"), hover_color=("#D8DADF", "#3A3A3A"), font=("Segoe UI", 12, "bold"), command=self.tela_feed)
        ...
        if eh_meu_perfil:
            ...
            btn_mudar_foto = ctk.CTkButton(frame_linha_inputs, text="🖼️ Escolher Foto", width=120, height=30, fg_color=("#E4E6EB", "#2A2A2A"), text_color=("#000000", "#FFFFFF"), hover_color=("#D8DADF", "#3A3A3A"), font=("Segoe UI", 11, "bold"), command=lambda: self.escolher_foto_perfil(lbl_status_foto))
            ...
            btn_deletar_conta = ctk.CTkButton(frame_tema, text="⚠️ Excluir Minha Conta", height=30, fg_color=("#FF4D4D", "#CC3333"), hover_color=("#FF3333", "#992222"), font=("Segoe UI", 11, "bold"), corner_radius=8, command=self.deletar_usuario)
            btn_deletar_conta.pack(side="right", padx=5)
    ...
    def escolher_foto(self):
        arquivo_origem = filedialog.askopenfilename(title="Anexar imagem ao post", filetypes=[("Imagens", "*.png *.jpg *.jpeg *.gif")])
        if arquivo_origem:
            try:
                _, extensao = os.path.splitext(arquivo_origem)
                ...
                self.btn_foto.configure(text="Foto Anexada!", fg_color=("#1F6AA5", "#4AA3DF"), text_color="#FFFFFF")
            except Exception as e:
                self.caminho_imagem = None
                self.btn_foto.configure(text="Falha ao Anexar", fg_color=("#FF4D4D", "#CC3333"), text_color="#FFFFFF")
    ...
    def renderizar_lista_posts(self, lista_filtrada, de_perfil=False):
        ...
        for post in lista_filtrada:
            ...
            btn_link_autor = ctk.CTkButton(
                cabecalho, text=f"{post['autor']}", font=("Segoe UI", 15, "bold"), text_color=("#1F6AA5", "#4AA3DF"),
                fg_color="transparent", hover_color=("#F0F2F5", "#2A2A2A"), width=0, height=25,
            ...
            if post["autor"] == usuario_logado: 
                btn_deletar = ctk.CTkButton(cabecalho, text="🗑️", width=30, height=30, fg_color="transparent", text_color="#FF4D4D", hover_color=("#FEE2E2", "#2D1F1F"), font=("Segoe UI", 14), command=lambda p=post: self.deletar_post(p, de_perfil))
            ...
            btn_like = ctk.CTkButton(rodape, text=f"❤️  {post['curtidas']}", width=80, height=32, fg_color="transparent", text_color=("#E02424", "#F87171"), hover_color=("#FEE2E2", "#2D1F1F"), font=("Segoe UI", 13, "bold"), corner_radius=8, command=lambda p=post: self.curtir(p, de_perfil))
            ...
            if "comentarios" in post and post["comentarios"]:
                for cmt in post["comentarios"]:
                    lbl_cmt = ctk.CTkLabel(frame_comentarios, text=f"💬 {cmt['autor']}: {cmt['texto']}", font=("Segoe UI", 12), wraplength=600, justify="left")
            ...
            btn_enviar_cmt = ctk.CTkButton(barra_digitar_cmt, text="Comentar", width=80, height=28, font=("Segoe UI", 11, "bold"), corner_radius=6, command=lambda p=post, e=entry_cmt: self.adicionar_comentario(p, e, de_perfil))
